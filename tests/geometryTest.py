...
class CfhtGeomTestCase(unittest.TestCase):
    """A test case for Cfht camera geometry"""
    
    def setUp(self):
        CfhtGeomTestCase.ampSerial = [0] # an array so we pass the value by reference
...
def suite():
    ...
    suites = []
    suites += unittest.makeSuite(CfhtGeomTestCase)
